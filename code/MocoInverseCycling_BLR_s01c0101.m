...
function MocoInverseCycling_BLR_s01c0101()
...
% parameters.
modelProcessor = ModelProcessor('/Users/rosswilkinson/Google Drive/projects/moco-cycling-standing/results/BLR_s01modelScaled.osim');
modelProcessor.append(ModOpAddExternalLoads('/Users/rosswilkinson/Google Drive/projects/moco-cycling-standing/setup/BLR_s01c0101setupExternalLoads.xml'));
...
% base table.
inverse.setKinematics(TableProcessor('/Users/rosswilkinson/Google Drive/projects/moco-cycling-standing/results/BLR_s01c0101inverseKinematics.mot'));
% Initial time, final time, and mesh interval.
inverse.set_initial_time(7.15);
inverse.set_final_time(7.65);
inverse.set_mesh_interval(0.01);
% By default, Moco gives an error if the kinematics contains extra columns.
% Here, we tell Moco to allow (and ignore) those extra columns.
inverse.set_kinematics_allow_extra_columns(true);
% Set maximum number of iterations
inverse.set_max_iterations(100);
% Loosen the convergence and constraint tolerances
inverse.set_convergence_tolerance(3e1);
inverse.set_constraint_tolerance(3e-3);
% Solve the problem and write the solution to a Storage file.
solution = inverse.solve();
solution.getMocoSolution().write('3DCycling_MocoInverse_solution_BLR_s01c0101.sto');
% Generate a report with plots for the solution trajectory.
model = modelProcessor.process();
report = osimMocoTrajectoryReport(model, ...
        '3DCycling_MocoInverse_solution_BLR_s01c0101.sto', 'bilateral', true);
