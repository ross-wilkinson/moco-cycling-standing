...
function MocoInverseCycling_s02c0101()
...
% parameters.
modelProcessor = ModelProcessor('s02modelScaled.osim');
modelProcessor.append(ModOpAddExternalLoads('s02c0101setupExternalLoads.xml'));
...
% base table.
inverse.setKinematics(TableProcessor('s02c0101inverseKinematics.mot'));
% Initial time, final time, and mesh interval.
inverse.set_initial_time(4.39);
inverse.set_final_time(4.88);
...
solution.getMocoSolution().write('3DCycling_MocoInverse_solution_s02c0101.sto');
% Generate a report with plots for the solution trajectory.
model = modelProcessor.process();
report = osimMocoTrajectoryReport(model, ...
        '3DCycling_MocoInverse_solution_s02c0101.sto', 'bilateral', true);
