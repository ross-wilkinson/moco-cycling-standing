function editSetupExternalLoadsXml(datDir,modDir,resDir,setDir,filename,RFxyz_r,RFxyz_l,Pxyz_r,Pxyz_l)
...
dataMot = vertcat(timeVector, RFxyz_r, Pxyz_r, RFxyz_l, Pxyz_l);  
k = find(isnan(dataMot));
dataMot(k) = 0;

% Change directory to subject setup folder and open file.
cd(setDir)
fileId = fopen([filename 'externalLoads.mot'],'w');
